...
-- The 'smartindent' behavior was originally designed for C-like languages,
-- where indentation can be inferred from simple syntactic cues. For example,
-- if a line ends with a `{`, the next line is automatically indented.
-- Similarly, lines starting with `}` are outdented. These rules work well
-- enough for structured code, but they're based on surface-level patterns, not
-- actual syntax trees.
--
